...
@MainActor
struct AmenityViewSnapshotTests {
    ...
    @Test("Normal") func testNormal() throws {
        ...
        hostingVC.view.frame = CGRect(x: 0, y: 0, width: 640, height: 480)

        assertSnapshot(of: hostingVC, as: .image)
    }
    ...
    @Test("Normal content on small screen") func testSmall() throws {
        ...
        let hostingVC = UIHostingController(rootView: view)
        hostingVC.view.frame = CGRect(x: 0, y: 0, width: 320, height: 120)

        assertSnapshot(of: hostingVC, as: .image, record: recordAll)
    }
